...
[ISSUE]
    ...
    6. 별점은 별 반개를 1점으로 10점 만점 하자!

[데이터 다루는 방식]
    디바이스 안/밖 기준, 두가지로 나눌 수 있음
        온디바이스
            POST: 아예 없거나(술 정보) ~ 빈번할 수 있음(도감페이지)
            GET: 디바이스에 데이터가 없거나 명시적 업데이트 요청시
            예시
                특정 유저의 행위에 대한 데이터 변동이 해당 유저에게만 적용 (개인 정보)
                유저가 무슨짓을 해도 변하지 않는 데이터
        서버
            POST: 필요할 떄 마다
            GET: 필요할 떄 마다
            예시
                유저의 상호작용이 모든 유저가 보는 페이지를 변경하는 경우 (리뷰, 별점)
    데이터 로딩 순서
        1. 브라우저스토리지 => 램 
        2. 디바이스롬 => 램
        3. 서버 => 램
    ...
    디바이스 캐싱
        ...
        이를 처리 가능한 런타임이 존재함
        정적 할당 (썸네일 이미지 50메가, 그외 30메가 등)
`
const 기본프로토콜형태 = [
    요청타입 | 응답결과, // 응답결과는 문제 없다면 null
    null | {} // 데이터
]
const 배열표현이맘에안들면이렇게바꾸시죠 = {
    "type": 요청타입 | 응답결과,
    "content": null | {}
}
...
const 계정가입_요청 = [
    "post-acct",
    {
        ...
        "name": "술취한 참새 29819", //임의 생성된 닉네임 (이부분은 서버에서 생성할지말지 논의)
...
const 계정로그인_응답 = [
    ...
    null | "email" | "pwd",
    null | {
        "token": "c2778d43400d656e25671b58a5f6c2b82278c5f5", // 이것으로 세션 진위여부를 확인하게 됨
        // 로그인 시점에 생성됨 (나이브하게)
        // 토큰은 로그아웃 시점에 만료됨 (나이브하게)
        // 토큰이 맞지 않으면 강제 로그아웃 시킬것임 (위 시나리오에선 복수 기기 접속시 과거 기기에서는 로그아웃 될 듯)
        "book-info": [
            [132, 499] // 인증 술 목록
            [132, 499, 1420, 802] // 찜 술 목록
            [132, 499, 1420] // 평가 술 목록
            [132, 499, 1420] // 리뷰 술 목록
        ]
    }
]
...
const 술정보_응답 = [
    ...
    null | {
        ...
        "my": {
            "star": null | 20, // 내가 준 별점
            "taste": null | [5, 3, 2, 4, 1], // 앱에서 쓰는 5개 맛 척도
            "pairing": null | ["육개장", "명태전"],
            "scent": null | ["풀잎", "새집"]
        },
        "sample": null | {
            "star": 4.2, // 평균 별점
